            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 4),spacing: 12) {
                ForEach(icons, id: \.self) { icon in
                    Image(systemName: "app.fill")
                        ...
                        .foregroundStyle(viewModel.user.profileImage == icon ? Color.blue : Color.white)
                        .shadow(radius: 5)
                        .overlay {
                            Image(systemName: icon)
                                .resizable()
                                .frame(width: 40, height: 40)
                                .foregroundStyle(viewModel.user.profileImage == icon ? Color.white : Color.black)
                        }
                        .onTapGesture {
                            withAnimation(.spring(response: 0.3)) {
                                viewModel.user.profileImage = icon
                            }
                        }
                }
            }
