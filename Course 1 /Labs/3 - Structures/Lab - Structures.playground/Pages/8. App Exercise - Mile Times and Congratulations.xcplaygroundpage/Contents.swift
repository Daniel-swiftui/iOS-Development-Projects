...
struct RunningWorkout {
    ...
    init(distance: Double, time: Double, elevation: Double) {
        ...
        averageMileTime = (distance / 1600) * time //not sure if this math is correct
    }
    
}

var running = RunningWorkout(distance: 1600, time: 3, elevation: 0)
...
struct Steps {
    var steps: Int {
        willSet {
            if newValue == goal {
                print("Congratulations you hit your goal!")
            }
        }
    }
    var goal: Int
...
var steps = Steps(steps: 9998, goal: 10_000)
steps.takeStep()
steps.takeStep()
