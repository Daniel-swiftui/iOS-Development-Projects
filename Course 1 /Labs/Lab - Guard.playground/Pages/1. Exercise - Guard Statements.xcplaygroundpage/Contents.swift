...
func createUser() -> User? {
    guard let fistName = firstNameTextField.text, let lastName = lastNameTextField.text, let age = ageTextField.text else {
        return nil
    }
    return User(firstName: fistName, lastName: lastName, age: age)
}
...
if let user = createUser() {
    print("\(user.firstName) \(user.lastName) is \(user.age) years old ")
}
